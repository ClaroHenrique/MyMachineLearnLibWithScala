...
object IrisDataSet{
    ...
    private
    def proccessLine(line: String): List[AnyVal] = {
        ...
        List(sampleData(0),sampleData(1),sampleData(2),sampleData(3),sampleData(4))
    }

    private
    def encodeLabel(label: String): Char = {
        label match {
            case "Iris-setosa"      => 's'
    ...
    private
    def extractFeatures(sample: List[AnyVal]): List[AnyVal] = {
        List(sample(0),sample(1),sample(2),sample(3))
    }

    private
    def extractLabel(sample: List[AnyVal]): AnyVal = {
        sample(4)
    }
}